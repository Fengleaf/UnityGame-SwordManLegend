...
namespace DigitalRuby.WeatherMaker
{
    ...
    public static class SphereCreator
    {
        ...
        public static Mesh Create(int resolution, UVMode uvMode)
        {
            if (resolution < 0)
            {
                resolution = 0;
            ...
            MeshData m = CreateMesh(verticesCapacity, trianglesCapacity, uvMode.ToString());
            CreateInternal(ref m, resolution, uvMode);

            return m.Mesh;
        }

        private static MeshData CreateMesh(int verticesCapacity, int trianglesCapacity, string meshNameSuffix)
        {
            MeshData m = new MeshData
            {
                Mesh = new Mesh { name = "WeatherMakerSkySphere_" + meshNameSuffix },
